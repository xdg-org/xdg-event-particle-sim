...
rank_cell_tracks(const std::vector<MeshID>& volumes,
                 const std::vector<double>& cell_tracks)
{
  std::vector<std::pair<MeshID, double>> ranked;
  ranked.reserve(volumes.size());
  for (const MeshID volume : volumes) {
    ranked.emplace_back(
      volume, cell_tracks.at(static_cast<std::size_t>(volume)));
  }
...
const std::vector<std::string>& summary_columns()
{
  static const std::vector<std::string> columns {
    ...
    "profile_rays_traced",
    "profile_transport_throughput_rays_per_s",
...
maximum_cell_track(const std::vector<std::pair<MeshID, double>>& ranked)
{
  if (ranked.empty()) {
    return {"", ""};
  }
  return {
    fmt::format("{}", ranked.front().first),
    fmt::format("{:.17g}", ranked.front().second)
  };
}
...
{
  // Report complete-transport and ray-trace-only throughput separately.
  const auto& profiling = sim_data.profiling;
  const double ray_trace_throughput =
    profiling.total_ray_trace_s > 0.0
      ? static_cast<double>(profiling.rays_traced) / profiling.total_ray_trace_s
      : 0.0;
  const double transport_throughput =
    profiling.transport_s > 0.0
      ? static_cast<double>(profiling.rays_traced) / profiling.transport_s
      : 0.0;
  ...
  const auto ranked_cell_tracks =
    rank_cell_tracks(mm->volumes(), sim_data.cell_tracks);

  const auto num_cell_tracks_to_print =
    std::min<std::size_t>(5, ranked_cell_tracks.size());

  const auto [max_cell_track_volume_id, max_cell_track_length] =
    maximum_cell_track(ranked_cell_tracks);
  ...
  if (format == "csv") {
    const std::vector<std::string> values {
      ...
      fmt::format("{}", transport_throughput),
      fmt::format("{}", ray_trace_throughput),
      fmt::format("{}", profiling.average_ray_launch_throughput),
      max_cell_track_volume_id,
      max_cell_track_length
    };

    output << fmt::format("{}\n", fmt::join(summary_columns(), ","));
    output << fmt::format("{}\n", fmt::join(values, ","));
    return;
  }
  ...
  output << "Execution             : event-based GPU\n";
  ...
  output << "Transport throughput  : " << transport_throughput << " rays/s\n";
  output << "Avg launch throughput : "
         << profiling.average_ray_launch_throughput << " rays/s\n";
  output << "Ray-trace throughput  : " << ray_trace_throughput << " rays/s\n";
  output << "----------------------------------------\n";
...
{
  const auto& profiling = sim_data.profiling;
  const double transport_throughput = profiling.transport_s > 0.0
    ? static_cast<double>(profiling.rays_traced) / profiling.transport_s
    : 0.0;

  const auto& volumes = sim_data.xdg_->mesh_manager()->volumes();
  const auto ranked_cell_tracks = rank_cell_tracks(volumes, sim_data.cell_tracks);
  const auto num_cell_tracks_to_print =
    std::min<std::size_t>(5, ranked_cell_tracks.size());
  const auto [max_cell_track_volume_id, max_cell_track_length] =
    maximum_cell_track(ranked_cell_tracks);
  ...
  if (format == "csv") {
    const std::vector<std::string> values {
      ...
      metadata.ray_tracing_library,
      "history-based",
      fmt::format("{}", sim_data.num_threads_),
      fmt::format("{}", metadata.model_num_volumes),
      fmt::format("{}", metadata.model_num_surfaces),
      fmt::format("{}", metadata.model_num_volume_elements),
      fmt::format("{}", metadata.model_num_vertices),
  ...
  output << "Mesh library          : " << metadata.mesh_library << "\n";
  output << "Ray tracing library   : " << metadata.ray_tracing_library << "\n";
  output << "Execution             : history-based CPU\n";
  output << "CPU threads           : " << sim_data.num_threads_ << "\n";
  output << "Volumes               : " << metadata.model_num_volumes << "\n";
  output << "Surfaces              : " << metadata.model_num_surfaces << "\n";
  output << "Volume elements       : " << metadata.model_num_volume_elements << "\n";
  output << "Vertices              : " << metadata.model_num_vertices << "\n";
  ...
  output << "Transport time        : " << profiling.transport_s << " s\n";
  output << "----------------------------------------\n";
  output << "Reached max events    : "
         << profiling.particles_reached_max_events << "\n";
  output << "Particles dead        : " << profiling.particles_dead << "\n";
  output << "Particles lost        : " << profiling.particles_lost << "\n";
  output << "Rays traced           : "
         << fmt::format("{:.6e}", static_cast<double>(profiling.rays_traced))
                          ...
                          const std::vector<double>& cell_tracks)
{
  ...
  for (const MeshID volume : volumes) {
    cell_track_ofstream << fmt::format(
      "{},{:.17g}\n", volume,
      cell_tracks.at(static_cast<std::size_t>(volume)));
  }
}
